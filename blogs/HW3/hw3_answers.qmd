...
Taking logs (constants in $Y_i!$ drop out of optimization):
...
$$
\frac{d\ell}{d\lambda} = \sum_{i=1}^{n} \left( \frac{Y_i}{\lambda} - 1 \right) = \frac{n\bar{Y}}{\lambda} - n.
$$

Setting this to zero gives the closed-form MLE $\hat\lambda_{\text{MLE}} = \bar{Y}$ — the sample mean, which equals the Poisson mean parameter.
...
The **log link** is standard: $\lambda_i$ must be positive, but the linear predictor $X_i'\beta$ can be any real number. The exponential maps the real line onto $(0, \infty)$.

### Log-likelihood with covariates

Substituting $\lambda_i = \exp(X_i'\beta)$ into the Poisson pmf and summing across firms,
...
p_mle = 2 * (1 - sm.distributions.norm.cdf(np.abs(z_mle)))
```
